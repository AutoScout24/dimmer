...
object ToggleControllerJsonCommands {
  ...
  case class ActivationBody(percentage: Int, attributes: Map[String, Seq[String]] = Map.empty) {
  ...
  implicit val activationBodyReads: Reads[ActivationBody] = (
    (JsPath \ "rollout" \ "percentage").read[Int](min(1) keepAnd max(100)) and
  ...
  val activationBodyWrites: Writes[ActivationBody] = (
    (JsPath \ "rollout" \ "percentage").write[Int] and
  ...
  val sampleActivation = ActivationBody(42, Map("country" -> Seq("de-DE", "de-AT")))
